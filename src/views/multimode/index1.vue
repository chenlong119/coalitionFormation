      ...
      <el-col :span="12">
        ...
        width: circleSizes[index] + 'px',
                    ...
                    top: fixedCirclePositions[index].top + 'px'}"
                     ...
                     @mouseover="event => showTooltip(event, index,loginbusinessId.value)"
              ...
              <div class="el-table el-table--enable-row-hover el-table--medium">
    <div class="rectangle">
        <div v-for="(color, index) in state.circleColors" :key="index"
            :class="{ 'circle': true, 'flashing': state.circleFlashing[index] }"
            :style="{ backgroundColor: color, width: state.circleSizes[index] + 'px', height: state.circleSizes[index] + 'px' }"
            v-show="state.circleVisible[index]"
            @mouseover="event => showTooltip(event, index)"
    @mouseout="event => hideTooltip(event)"
    @click="showMessage(state.taskIds[index])">
    <span>任务{{ state.taskIds[index] }}</span>
        </div>
        <div class="custom-tooltip">
            蓝色代表家电产业链任务<br/>紫色代表汽车产业链任务<br/>红色闪烁表示任务有突发情况<br/>点击任务节点查看详情
        </div>
...
const loginbusinessName = ref('');
const loginbusinessId = ref('');
const isLoginBusiness = ref('');

const pageSize = ref(5);
; // 每页显示的行
const totalItems = ref(0);
// const initCircleColors = () => {
//   return new Array(10).fill('#A9C9FF').map((_, index) => {
//     if (index < 4) return '#EAD9E0'; // 紫色
//     if (index < 8) return '#CDE0FF'; // 蓝色
...
// const circleFlashing = ref(new Array(10).fill(false));
...
const circleSizes = ref(new Array(10).fill(100)); // 示例大小
const state = reactive({
  tasks: [],
    taskIds: [],
        ...
        // 准备圆圈的相关数据
        function prepareCircles(tasks) {
          state.taskIds = tasks.map(task => task.id); // 存储每个任务的ID
    state.circleColors = tasks.map(task => getColorByChainId(task.chainId));
    state.circleSizes = tasks.map(() => 50); // 假设所有圆圈大小一样
    state.circleFlashing = tasks.map(() => false); // 根据您的逻辑设置
...
onMounted(async () => {
  getActiveTasks().then(response => {
                // 假设response就是你需要的任务数组
                state.tasks = response;
                prepareCircles(response);
            }).catch(error => {
                console.error('获取任务数据失败', error);
            });
  try {
    const response = await getCompanyNow();
    if (response && response.length > 0) {
      // 假设您只关心列表的第一个元素
      loginbusinessName.value = response[0].business_name;
      loginbusinessId.value = response[0].business_id;
    }
  } catch (error) {
  ...
  // 获取所有 BusinessLocations
  const responseLocations = await fetchBusinessLocations();
  // 过滤 businessLocations
  businessLocations.value = responseLocations.filter(location =>
    ['1', '2', '3', '4', '5', '6'].includes(location.businessId)
  );
  // 获取所有 BusinessDirections
  const allDirections = await fetchBusinessDirections();
  // 过滤 businessDirections
  const filteredDirections = allDirections.filter(direction =>
    ['1', '2', '3', '4', '5', '6'].includes(direction.sourceId) &&
    ['1', '2', '3', '4', '5', '6'].includes(direction.goalId)
  );

  // 获取历史数据
  await Promise.all(filteredDirections.map(async direction => {
    if (direction.historyId) {
      const historyData = await fetchBusinessHistory(direction.historyId);
      direction.history = historyData;
    }
  }));

  businessDirections.value = filteredDirections;
...
const showTooltip = async (event, index) => {
  ...
  // 将产业链信息添加到状态信息之前
  event.target.title = `任务${taskId}: ${loginbusinessName.value}${taskName}\n${industryChainInfo}${statusMessage}`;
}
...
const initializeChart = () => {
  ...
  console.log(links);
   // 更新 selectedHistory，以便在图表加载时自动显示相关历史数据
  selectedHistory.value = businessDirections.value
    .filter(direction => direction.sourceId === loginbusinessId.value || direction.goalId === loginbusinessId.value)
    .flatMap(direction => (direction.history|| []).map(h => ({
      ...h,
      taskName: h.taskName,
      deliveryDate: h.deliveryDate,
      deliveryExperience: h.deliveryExperience
    })));
    console.log("selectedHistory before initializeChart:", selectedHistory.value);
  const option = {
    tooltip: {},
    ...
    series: [{
      ...
      data: businessLocations.value.map(location => {
        // 判断当前节点是否为 loginbusinessId 对应的节点
        const isLoginBusiness = location.businessId === loginbusinessId.value;
        ...
        return {
          ...
          x: location.x,
          y: location.y,
          symbol: isLoginBusiness ? 'diamond' : 'circle',
          symbolSize: isLoginBusiness ? 50 : 45,
          itemStyle: {
            color: isLoginBusiness ? 'red' : getColorByGroupId(location.businessGroupId)
          },
          // ...保留其他属性...
          tooltip: {
  ...
  //       symbol: isLoginBusiness ? 'star' : 'circle',  // 如果是登录业务ID，使用星星标记
  //     symbolSize: isLoginBusiness ? 50 : 45,  // 增大星星大小
  //     itemStyle: {
  //       color: isLoginBusiness ? 'red' : getColorByGroupId(location.businessGroupId) // 如果是登录业务ID，使用红色
  //     },
  //       tooltip: {
  //         formatter: `节点信息：<br/>${location.businessId}---${location.businessName}<br/>所属企业群编号：${location.businessGroupId}<br/>企业群：${location.businessGroupName}`
  //       }

...
<el-form-item label="实收数量">
  ...
  emitFeedbackChange(field, value) {
    if (field === 'actualDeliveryTime') {
      // 创建一个新的 Date 对象，并调整时区
      let localDate = new Date(value);
      localDate.setMinutes(localDate.getMinutes() + localDate.getTimezoneOffset());
      // 更新 actualDeliveryTime 字段为只包含日期的字符串
      this.feedbackForm.actualDeliveryTime = localDate.toISOString().split('T')[0];
    }

    // 发送更新的数据
    this.$emit('feedbackChanged', { field, value: this.feedbackForm[field] });

    // 计算 supplyRate 和 deliveryTimeDifference
    if (field === 'receivedQuantity' || field === 'actualDeliveryTime') {
